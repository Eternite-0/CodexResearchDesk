...
### Step 0: Enforce CodexResearchDesk Decision Gate

Before detecting GPUs, syncing code, editing launch scripts, or starting any process, run:

```powershell
python .\tools\decision_gate.py latest .\projects\<project-slug> --mode experiment
```

If the gate blocks, stop. `STATIC_ONLY` permits only static work and does not permit a small training run. `NEEDS_MORE_EVIDENCE`, `NO_GO`, missing JSON, invalid JSON, or stale decisions block experiments. `USER_OVERRIDE` is accepted only when the user explicitly requested override and the gate passes with `--allow-override`.
