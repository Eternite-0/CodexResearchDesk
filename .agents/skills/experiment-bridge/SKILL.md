...
## CodexResearchDesk Preflight Gate

Before parsing, implementing, deploying, or running any experiment, enforce the local Decision Memo gate:

```powershell
python .\tools\decision_gate.py latest .\projects\<project-slug> --mode experiment
```

If the command blocks, stop and report the recorded reason. Do not launch a smaller sanity run, pilot, GPU job, or long-running command unless the verdict is `GO` or the user explicitly provides a recorded `USER_OVERRIDE` accepted by `--allow-override`.
